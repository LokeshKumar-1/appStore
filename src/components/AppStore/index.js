...
class AppStore extends Component {
  ...
  filteringApps = () => {
    ...
    const filteredAppList = appsList.filter(item => item.category === tabId)
    return filteredAppList
  }

  onchangeEventTriggered = event => {
    const {tabId} = this.state
    this.setState({tabId, userEnteredValue: event.target.value})
  }

  changingStateValue = Id => {
    this.setState({tabId: Id})
  }

  render() {
    const {tabId, userEnteredValue} = this.state
    const filteredAppList = this.filteringApps()
    console.log(tabId, userEnteredValue)
    const filteredUsingInputValue = filteredAppList.filter(item =>
      item.appName.toLowerCase().includes(userEnteredValue.toLowerCase()),
    )
      ...
      <div className="bg-container">
        ...
        <div className="input-container">
          <input
            ...
            placeholder="search"
            onChange={this.onchangeEventTriggered}
          />
          <img
        ...
        <ul className="tabItemContainer">
          {tabsList.map(item => (
            <TabItem
              item={item}
              key={item.tabId}
              changingStateValue={this.changingStateValue}
              isSelected={tabId === item.tabId}
            />
          ))}
        </ul>

        <ul className="appItemContainer">
          {filteredUsingInputValue.map(item => (
            <AppItem item={item} key={item.appId} />
          ))}
        </ul>
      </div>
    )
